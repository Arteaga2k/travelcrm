...
class Constants extends Crmcontroller {
	...
	private function find(){
		...
		if ($this->form_validation->run() == True){
			$search_rule = array();
			if($this->input->post('code')) $search_rule['like']['_constants.code'] = $this->input->post('code');
			if($this->input->post('name')) $search_rule['like']['_constants.name'] = $this->input->post('name');
			if($this->input->post('archive')==0) $search_rule['where']['_constants.archive'] = $this->input->post('archive');			
