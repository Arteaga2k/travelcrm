...
class Interests_model extends Crmmodel{
	...
	public function get_ds(){
		$this->db->select('SQL_CALC_FOUND_ROWS _interests.rid as rid,
							_interests.interests_name as interests_name,
							DATE_FORMAT(_interests.modifyDT, \'%d.%m.%Y\') as modifyDT,
							_interests.descr as descr, _interests.archive', False);
		$this->db->from('_interests');
		...
		$query = $this->db_get('_interests');
		return $query->num_rows()?$query->result():array();
	}
	
	public function get_edit($rid){
		$this->db->select('_interests.rid as rid,
							_interests.interests_name as interests_name,
							DATE_FORMAT(_interests.modifyDT, \'%d.%m.%Y %H:%i\') as modifyDT,
							_interests.owner_users_rid,
							_interests.descr as descr, _interests.archive', False);
		$this->db->from('_interests');
		$this->db->where(array('_interests.rid'=>$rid));
		$query = $this->db_get('_interests');
		return $query->num_rows()?$query->row():False;
	}
	
	public function create_record(){
		$ins_arr = array('interests_name'=>$this->ci->input->post('interests_name'),
		...
		$this->db->trans_begin();
		$this->db->insert('_interests', $ins_arr);
	...
	public function update_record(){
		$update_arr = array('interests_name'=>$this->ci->input->post('interests_name'),
							'descr'=>$this->ci->input->post('descr'),
							'archive'=>$this->ci->input->post('archive'),
							'modifier_users_rid'=>get_curr_urid());
		$this->db->set('modifyDT', 'now()', False);
		$this->db->trans_begin();
		$this->db->update('_interests', $update_arr, array('rid'=>$this->ci->input->post('rid')));
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
		}else{
	...
	public function remove_items(){
		...
		foreach($this->ci->input->post('row') as $rid){
			$this->db->delete('_interests', array('rid'=>$rid));	
		}
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
	...
	public function check_unique($val, $type='code', $rid=null){
		...
		else $this->db->where(array('interests_name'=>$val));
	...
	public function move_record(){
		...
		$this->db->set('modifyDT', 'now()', False);
		$this->db->trans_begin();
		$this->db->update('_interests', $update_doc, array('_interests.rid'=>$this->ci->input->post('rid')));
		if ($this->db->trans_status() === FALSE){
    		$this->db->trans_rollback();
    		return False;
		}else{
	...
	public function get_list(){
		$this->db->select('*');
		$this->db->from('_interests');
		$this->db->order_by('_interests.interests_name');
		$query = $this->db->get();
		return $query->num_rows()?$query->result():array(); 
	}	
