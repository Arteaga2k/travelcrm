...
class Rooms extends Crmcontroller {
	...
	private function find(){
		...
		if ($this->form_validation->run() == True){
			$search_rule = array();
			if($this->input->post('code')) $search_rule['like']['_rooms.code'] = $this->input->post('code');
			if($this->input->post('room_name')) $search_rule['like']['_rooms.room_name'] = $this->input->post('room_name');	
			if($this->input->post('archive')==0) $search_rule['where']['_rooms.archive'] = $this->input->post('archive');		
